...
隔离复测已确认 Mason 能安装 `lua-language-server`、`clangd`、`stylua` 和 `tree-sitter-cli`；clangd
可以完成 C buffer attach。相同环境中的 lua-language-server 可执行文件存在且会自动启动，但 180 秒
内没有完成 initialize，因此 Lua LSP 的纯净安装状态仍未通过，不能由包目录存在代替。
