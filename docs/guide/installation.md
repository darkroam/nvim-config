...
0.12.3 隔离复测已确认 Mason 能安装 `lua-language-server`、`clangd`、`stylua` 和
`tree-sitter-cli`；0.11.3 和 0.11.7 也分别确认 Mason 可安装 `lua-language-server` 3.18.2-dev 与
...
python3 scripts/check-compat.py \
  ...
  --case 0.10.4=/path/to/nvim-0.10.4 \
  --case 0.11.3=/path/to/nvim-0.11.3 \
  --case 0.11.7=/path/to/nvim-0.11.7 \
  --case 0.12.3=/path/to/nvim-0.12.3
```
