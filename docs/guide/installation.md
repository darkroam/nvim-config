...
| --- | --- | --- |
| 0.10.x、0.11.0-0.11.2 | `stylua` | 禁用 |
| 0.11.3+ | `stylua`、`lua-language-server`、`clangd` | 禁用 |
| 0.12+ | 上述三项和 `tree-sitter-cli` | `lua`、`c`、`commonlisp` |
...
成功但外部命令缺失，`FAILED` 表示 Mason/parser 安装或最终验证失败。可用
`require("darkroam.bootstrap").last_report()` 读取最近一次结构化报告；没有运行过时返回 `nil`。

0.12.3 已在空 Mason/parser 的隔离 data 中通过统一命令安装 `lua-language-server`、`clangd`、`stylua`、
`tree-sitter-cli` 和三个 parser；随后幂等复跑时全部跳过，专用日志为空。0.10.4 在同一
数据上实际执行时只检查并跳过 StyLua，没有触发 LSP 或 parser。0.11.3 和 0.11.7 也分别确认 Mason
可安装 `lua-language-server` 3.18.2-dev 与
...
当前 C formatter 配置名是 `clang_format`，但 `clangd` 软件包不等于 `clang-format` 命令。当前机器缺少
独立 provider，因此统一命令的预期结果是 `PARTIAL`；纯净环境只能使用可用的 LSP fallback。补齐
provider 仍是 roadmap 项，不能记作已完成。
