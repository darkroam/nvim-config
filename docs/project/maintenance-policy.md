...
- 修改 `lua/darkroam/languages.lua` 时，同步架构、依赖、用户指南以及 LSP/Mason/Conform/
  Tree-sitter 消费关系。
- 修改 `lua/darkroam/lazy.lua`、`lua/darkroam/plugins/*.lua` 或 `lazy-lock.json` 时，同步
  `project/plugins.md`、兼容性、依赖和用户可见功能。
- 修改 `lua/darkroam/keymaps.lua` 或插件按键时，同步 `user/keybindings-zh.md`，并确认 provider
  确实已声明。
- 修改最低 Neovim 版本、Lazy/锁定策略或外部工具时，同步 README 和依赖文档。
...
- Lazy 直接声明是否全部出现在插件清单，锁文件是否覆盖全部受管插件；
...
Lazy manager、锁文件、Neovim 版本档位和遗留配置已经过方案确认。后续提高版本门槛、拆分多 lockfile、
恢复旧 provider 或改变降级功能仍是独立提案对象，活动状态见
