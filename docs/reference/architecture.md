...
- `lazy.lua` 的 manager pin 与 lockfile 的 `lazy.nvim` 条目是同一原子状态，任何时候都必须相等；
- 日常 data checkout 是可恢复机器状态，不是新 lock 或更新验收的来源；更新只能在隔离环境生成；
...
`scripts/check-lua-format.py` 通过 Git 清单限定输入，不扫描 untracked、插件、Mason 或临时目录。它先独立
检查 UTF-8 BOM、CRLF 和末尾换行，再按显式 `STYLUA`、PATH、当前 XDG data 下的 Mason provider 顺序
查找 StyLua，并执行 `.stylua.toml` 驱动的 `--check --verify`。provider 缺失或不可执行时返回 2；此时
编码预检即使成功，也没有全库格式通过结论。
