...
| 平台 shell | 核心 | Unix 使用 `zsh`；Windows 使用 `pwsh`，没有 PowerShell 7 时回退 `powershell`；分别服务于 `:!` 和 ToggleTerm |
...
ToggleTerm 和 `:!` 使用平台 shell：Unix 为 `zsh`，Windows 为 `pwsh` 或内置 `powershell`。Windows
还必须使用 Neovim 0.12 的 PowerShell `shellcmdflag`、`shellpipe`、`shellquote` 和 `shellxquote` 组合；
只把 `vim.opt.shell` 改成 `pwsh` 而保留 Windows 默认 `/s /c` 会导致命令和插件 build 失败。`:TermExec
cmd="..."` 可以按用户要求启动任意外部命令，但该入口不会让 Lazygit、Node、ncdu、htop、Python 或其他
临时目标自动成为仓库依赖；缺失行为属于该次命令调用。仓库不提供这些工具的全局 Lua helper、专用 user
command 或按键，也不把 `python` 别名当作 `python3` 的替代。
...
| 软件或能力 | 级别 | 用途 |
| --- | --- | --- |
| C compiler、`make` 及标准构建环境 | 功能必需 | Tree-sitter parser 或插件原生构建路径；Windows 还要让 Git `sh.exe` 在 PATH 中，以完成 LuaSnip `jsregexp` build |
| `tar`、`curl` | 功能必需 | nvim-treesitter `main` 下载和解包 parser source；现代 Windows 通常自带 `tar.exe` 和 `curl.exe`，仍须实际检查 |
...
Windows 原生部署还需要 PowerShell 7（推荐 `pwsh`，否则使用系统 `powershell`）、Git for Windows 的
`bin/sh.exe`、GNU `make` 和 `gcc`/`clang`/`zig` 中至少一个 C compiler。LuaSnip 的 Windows Makefile 会
读取 `OS=Windows_NT` 并链接 Neovim 的 Lua DLL；Git `sh.exe` 不在 PATH 时，Lazy 首次 build 可能只报告
任务失败而不影响基础启动，必须在 `:Lazy` 和完整输出中检查。Windows 的原生剪贴板由 Neovim provider
提供，不需要 `xclip` 或 `wl-copy`。
