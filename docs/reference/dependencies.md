...
| Mason | `stylua`；LSP 档位的 `lua-language-server`、`clangd` 和可选 `gopls`；Tree-sitter 档位的 `tree-sitter-cli` | 下载到 Mason data 目录，不跟踪；会把自己的 `bin` 加入 Neovim PATH；由用户显式 `:DarkroamBootstrap` 统一处理 |
...
`clang-format` 和 `gofmt` 等外部 formatter 只检查并报告，不调用系统包管理器；C 编译工具链同样不会
自动安装，缺失时由 parser 安装结果暴露失败。当前启用 C 而 `clang-format` 缺失时，bootstrap 应报告
`PARTIAL`；这与 clangd 可提供 LSP fallback 并不矛盾。
