...
| 插件 | 状态 | 配置和用户行为 |
| --- | --- | --- |
| `nvim-treesitter/nvim-treesitter` | 条件活动；`main`、`lazy=false`；首次安装缺陷 | 新版 setup、内置高亮/缩进、版本专用 parser 目录和 parser 命令；当前 `:TSUpdate` build hook 在首次安装时早于命令注册 |
...
2026-07-18 的 0.12.3 隔离纯净恢复确认：首次 `:Lazy! restore` 在 build 阶段输出
`Command not found: TSUpdate`，因为上游 `plugin/nvim-treesitter.lua` 尚未注册命令。第二次启动命令
存在，但不能消除首次安装失败；修复前不得把 Tree-sitter 的 clean-room 安装记为通过。
