...
| Normal | `,rr` | Telescope `find_files`，包含隐藏文件但尊重 ignore | Telescope；Neovim 0.11.7+ |
| Normal | `,dd` | Telescope `live_grep` | Telescope、`rg`；Neovim 0.11.7+ |
| Normal | `,bb` | Telescope buffers | Telescope；Neovim 0.11.7+ |
| Normal | `;t` | Telescope help tags | Telescope；Neovim 0.11.7+ |
| Normal | `;;` | 恢复上次 Telescope picker | Telescope；Neovim 0.11.7+ |
| Normal | `;e` | Telescope diagnostics | Telescope；Neovim 0.11.7+ |
| Normal | `,kk` | Telescope keymaps | Telescope；Neovim 0.11.7+ |
| Normal | `,xf` | 当前文件目录的 Telescope file browser | Telescope file-browser；Neovim 0.11.7+ |
...
Neovim 0.12 的 Tree-sitter Textobjects 还配置 `af`（function outer）和 `if`（function inner），适用于
Operator-pending 和 Visual mode；0.11 及以下不创建这两个插件映射。
...
只有 Neovim 0.11.3+ 且 LSP client 成功 attach 后才存在：
...
恢复这些功能时必须同步本表和 [`../project/plugins.md`](../project/plugins.md)。
